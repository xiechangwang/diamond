<template>
	<div id="app">
		<el-breadcrumb separator-class="el-icon-arrow-right" style="padding: 10px;margin-bottom:8px;border-bottom: 1px solid #eee;background: #eee;">
			<el-breadcrumb-item>新增活动页</el-breadcrumb-item>
		</el-breadcrumb>
		<div>
			<form @submit.prevent="submit" method="post" enctype="multipart/form-data" action="">
				...
				<UEditor :config="config" ref="ueditor" style="width: 96% !important;margin: 0 auto;"></UEditor>
			</form>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			submit(evt) {
				...
				this.$axios.post("/activity/createActivity", fd).then(res => {
					let msg=res.data;
					if(msg.code=="OK"){
						this.$message({
							type: 'success',
